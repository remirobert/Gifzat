...
- (instancetype)initWithSettings:(NSDictionary *)videoSettings;
{
    ...
    if (self) {
        ...
        NSDictionary *bufferAttributes = [NSDictionary dictionaryWithObjectsAndKeys:
                                          [NSNumber numberWithInt:kCVPixelFormatType_16LE565], kCVPixelBufferPixelFormatTypeKey, nil];
    ...
    [self.writerInput requestMediaDataWhenReadyOnQueue:mediaInputQueue usingBlock:^{
        while (YES){
            ...
            if ([self.writerInput isReadyForMoreMediaData]) {
                ...
                CVPixelBufferRef sampleBuffer = [self newPixelBufferFromCGImage:[[images objectAtIndex:i] CGImage] image:[images objectAtIndex:i]];
...
- (CVPixelBufferRef)newPixelBufferFromCGImage:(CGImageRef)image image:(UIImage *)imageOriginal
{
    ...
    CGSize dstSize = CGSizeMake(frameWidth, frameHeight);
    CGSize srcSize = [imageOriginal size];

    CGFloat marginWidth = (dstSize.width - srcSize.width) / 2;
    CGFloat marginHeight = (dstSize.height - srcSize.height) / 2;
    
//    CGContextDrawImage(context, CGRectMake(marginWidth, marginHeight, srcSize.width, srcSize.height), image);
