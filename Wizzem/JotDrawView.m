...
- (void)drawTouchBeganAtPoint:(CGPoint)touchPoint
{
    ...
    [self drawTouchMovedToPoint:touchPoint];
    [self drawTouchMovedToPoint:touchPoint];
    [self drawTouchMovedToPoint:touchPoint];
    [self drawTouchMovedToPoint:touchPoint];
}
...
- (void)drawBitmap
{
    ...
    if (self.pointsArray.count == 1) {
        ...
        touchPoint.strokeWidth = 1.0f * [self strokeWidthForVelocity:1.f];
...
{
    return self.strokeWidth - ((self.strokeWidth * (1.f - 1)) / (1.f + (CGFloat)pow((double)M_E, (double)(-((velocity - self.initialVelocity) / self.initialVelocity))))) - 0.3;
}
