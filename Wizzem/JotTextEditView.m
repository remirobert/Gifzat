...
- (instancetype)init
{
    if ((self = [super init])) {
        ...
        _textEditingInsets = UIEdgeInsetsMake(45.f, 0.f, 0.f, 0.f);
        ...
        [[NSNotificationCenter defaultCenter] addObserverForName:UIKeyboardWillChangeFrameNotification
                                                      ...
                                                      usingBlock:^(NSNotification *note){
                                                          ...
                                                          NSLog(@"height screen:  %f", CGRectGetHeight([UIScreen mainScreen].bounds));
                                                          NSLog(@"height keyboard:  %f", keyboardRectEnd.size.height);
                                                          
                                                          
                                                          CGFloat differenceHeight = CGRectGetHeight([UIScreen mainScreen].bounds) - CGRectGetWidth([UIScreen mainScreen].bounds) - 69;
                                                          
                                                          if (CGRectGetHeight(keyboardRectEnd) > differenceHeight) {
                                                              
                                                              differenceHeight = CGRectGetHeight(keyboardRectEnd) - differenceHeight;

                                                              [self.textContainer mas_updateConstraints:^(MASConstraintMaker *make) {
//                                                                  make.centerY.equalTo(self).offset(-differenceHeight);
                                                                  make.height.offset(CGRectGetHeight([UIScreen mainScreen].bounds) - keyboardRectEnd.size.height);
                                                                  make.centerY.offset(-(keyboardRectEnd.size.height / 2));
//                                                                make.bottom.equalTo(self).offset(-differenceHeight);
                                                              }];
